/* 이 소스는 에듀이노(Eduino)에 의해서 번역, 수정, 작성되었고 소유권 또한 에듀이노의 것입니다.
    ...
    Arduino Smart Car Source Code
    기존의 코드는 속도가 153으로 굉장히 느립니다. 하지만, 해당 코드를 통해 가장 빠르게 Line Tracing을
    진행할 수 있습니다. 직진, 좌회전, 우회전 파라미터를 추가하여 그때마다 바퀴의 속도를 조정한 것입니다.
    직진일 때는. 좌우의 바퀴가 최대속도로, 좌회전일때는, 왼쪽의 바퀴의 속도를 높이고 오른쪽 바퀴를 속도를 
    낮춘 것입니다. 해당 내용은 주석으로 자세하게 처리되어 있으니, 확인해보시기 바랍니다.
*/
